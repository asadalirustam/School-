...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Content (Fixed on lg screens, slide-over drawer on mobile/tablet) */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-300 flex flex-col h-full border-r border-slate-800 transition-transform duration-300 ease-in-out lg:translate-x-0 ${
          isOpen ? 'translate-x-0 shadow-2xl animate-slide-in' : '-translate-x-full lg:translate-x-0'
        }`}
      >
        {/* Brand Header */}
        <div className="h-16 flex items-center justify-between px-6 border-b border-slate-800 bg-slate-950 shrink-0">
          <div className="flex items-center space-x-3">
            ...
            <span className="font-semibold text-white tracking-wide truncate">EduManager Pro</span>
          </div>
        ...
        {/* Navigation Links */}
        <nav className="flex-1 px-4 py-4 space-y-1.5 overflow-y-auto scrollbar">
          {items.map((item) => (
            <NavLink
              key={item.name}
              to={item.path}
